/*
    Bibliografias das disciplinas

    No siga: é a tela Bibliografia do plano de ensino da disciplina

    sendo codigo da disciplina: { bibliografias }

    todas as bibliografias de todas as disciplinas listadas no arquivo disicplinas.js, não podendo repetir.

*/

/*  exemplo
    '': [
        '',
        '',
    ],
*/

module.exports = {
    'HSO003': [
        'SCHWARTZ, N. Noções de Direito. Juruá, 2009.',
        'PONCHIROLLI, O. Ética e Responsabilidade Social Empresarial. Juruá, 2007.',
        'PAESANI, L. M. Direito de Informática: comercialização e desenvolvimento internacional do software. Atlas, 2006.',
        'KRAUT, R; STORCK, A. Aristóteles – A Ética à Nicomaco. Artmed, 2009.',
        'REALE, M. Lições preliminares de direito. 27.ed. Saraiva, 2009.',
        'MASIERO, P C. Ética em Computação. EDUSP, 2008.',
        'FRAGOSO, João Henrique da Rocha. Direito Autoral - da Antiguidade a Internet. Quartier Latin, 2009.',
    ],
    'IIA002': [
        'Luiz A. M. Palazzo. Introdução a programação Prolog http://puig.pro.br/Logica/palazzo.pdf',
        'WOOLDRIDGE, Michael J. The Logical Modelling of Computational Multi- Agent Systems. PhD Thesis, Uk, University of Manchester, Faculty of Technology, August 1992. 163 p.;',
        'RICH, Elaine, KNIGHT Kevin, Inteligência Artificial. Makron Books, 1993. 722 p.;',
        'RUSSELL, S., NORVIG, P. Artificial Intelligence – A Modern Approach. 3rd. ed. New Jersey: Prentice-Hall, 2010;',
    ],
    'IED001': [
        'KOFFMANN, E. B. Objetos, abstração, estrutura de dados e projeto. LTC, 2008.',
        'ASCENCIO, A. F. G. Estrutura de dados: algoritmos, análise da complexidade e implementações em JAVA e C/C++. São Paulo: Pearson Prentice Hall, 2010.',
        'JANDL JUNIOR, Peter. Java - Guia do Programador. 2a. edição. São Paulo: Novatec, 2013.',
        'PEREIRA, Silvio do Lago. Estruturas de Dados Fundamentais – Conceitos e Aplicações. 12ª edição, 2ª reimpressão, São Paulo: Érica, 2009.',
        'KOFFMANN, E. B. Objetos, abstração, estrutura de dados e projeto. LTC, 2008.',
        'EDELWEISS, N; GALANTE, R. Estruturas de Dados. Livros Didáticos UFRGS, V.18. Bookman, 2009.',
    ],
    'ARG101': [
        'BRUZZI, Demerval Guilarducci. Gerência de Projetos. Editora SENAC, 2008.',
        'REIS, A M V; BECKER JR., L C; TONET, H. Desenvolvimento de Equipes. FGV, 2009.',
        'HERINGER, Monika Gomes. A dimensão humana na gestão do conhecimento. In: Congresso Nacional de Inteligência Competitiva, 2005, Brasília. Anais do Congresso Nacional de Inteligência Competitiva. Brasília, 2005.',
        'HOWARD, Robert et al. (Org.). Aprendizado Organizacional: Gestão de pessoas para a inovação contínua. Rio de Janeiro: Campus, 2000.',
        'KNOWLES, Malcolm S.; III, Elwood F. Holton; SWANSON, Richard A.. Aprendizagem de Resultados: uma abordagem prática para aumentar a efetividade da educação corporativa. Rio de Janeiro: Elsevier, 2009.',
    ],
    'HST002': [
        'WIENER, Norbert (1968). Cibernética e Sociedade: o uso humano de seres humanos. Traduzido por José Paulo Paes. 2 ed. São Paulo: Cultrix.',
        'RUSH, Michael (2006). Novas Mídias na Arte Contemporânea. São Paulo: Martins Fontes.',
        'RIFKIN, J. A Era do Acesso. São Paulo: Makron Books. 2001.',
        'VIRILIO, Paul (1999). A Bomba Informática. Traduzido por Luciano Vieira Machado. São Paulo: Estação Liberdade.',
        'SANTOS, L. G. Politizar as Novas Tecnologias. São Paulo, Editora 34, 2003.',
        'LIVRO VERDE - Sociedade da Informação no Brasil, in Ciência, Tecnologia e Inovação – desafios para a sociedade brasileira. Brasília: MCT, 2001.',
        'ARAÚJO, H. R., Tecnociência e cultura, São Paulo, Estação Liberdade, 1998.',
        'SANTAELLA, Lucia. Culturas e Artes do Pós-humano: da Cultura das Mídias à Cibercultura. S Paulo: Paulus, 2003.',
    ],
    'MMD001': [
        'SULLIVAN, Michael; MIZRAHI, Abe. Matemática Finita – Uma abordagem aplicada. LTC, 2006.',
        'SCHEINERMAN, E.R. Matemática Discreta: Uma Introdução. São Paulo: Cengage Learning, 2008.',
        'LIPSCHUTZ, Seymour, LIPSON, Marc. Matemática Discreta. Porto Alegre: Bookman, 2004.',
        'GERSTING, Judith L. Fundamentos Matemáticos para a Ciência da Computação. 5. ed. LTC, 2004.',
        'GARCIA LOPEZ, J; TOSCANI, L V; MENEZES, P B. Aprendendo Matemática Discreta com Exercícios. Coleção Livros Didáticos Informática UFRGS, V.19. Bookman, 2009.',
    ],
    'ITI004': [
        'BRAND, K. IT Governance based on COBIT 4.1: A Management guide. USA: Van Haren Publisher, 2008. LAHTI, C.; PETERSON, R. SARBANES – OXLEY COBIT e ferramentas open source. Alta books, 2006.',
        'FERNANDES, A ARAGON; ABREU, V. Implantando a Governança de TI. Brasport, 2008. MAGALHÃES, I. L.; PINHEIRO, W. Gerenciamento de Serviços de TI na Prática: Uma Abordagem com Base na ITIL. SP: Novatec, 2007. MANSUR, R. Governança Avançada de TI na Prática. Brasport, 2009.',
    ],
    'LIN006': [
        '',
        '',
    ],
    'ILP010': [
        '',
        '',
    ],
    'AAG001': [
        'MAXIMIANO, A. C. A. Introdução à Administração, SP: Atlas, 2000.',
        'COELHO, M. A essência da administração – conceitos introdutórios. Saraiva, 2008.',
        'CAULLIRAUX, H; CLEMENTE, R; PAIM, R. Gestão de Processos: Pensar, Agir e Aprender. Bookman, 2009.',
        'CERTO, Samuel C. Administração Moderna. Pearson Brasil, 2003.',
        'MORGAN, Gareth. Imagens da Organização. Atlas, 2007.',
        'ROBBINS, Stephen P.; DECENZO, David A. Fundamentos de Administração. Pearson, 2004.',
    ],
    'IAC001': [
        'STALLINGS, W. Arquitetura e organização de computadores. 5.ed. Prentice-Hall Brasil, 2008.',
        'TANENBAUM, A. S. Organização Estruturada de Computadores, 5ª Ed. Prentice Hall, 2007.',
        'TOCCI, R. J. Sistemas digitais: princípios e aplicações. 10.ed. Pearson Brasil, 2007.',
    ],
    'IAL002': [
        'EVARISTO, J. Aprendendo a programar: programando em Linguagem C (para iniciantes). São Paulo: Book Express, 2001.',
        'SALVETTI, D. D.; BARBOSA, L. M. Algoritmos. São Paulo: Makron Books, 1998.',
        'FORBELLONE, A. L. V.; EBERSPACHER, H. F. Lógica de programação: a construção de algoritmos e estruturas de dados. 3. ed. São Paulo: Prentice Hall, 2005.',
        'PIVA JUNIOR, D. et al. Algoritmos e programação de computadores. 1. ed. São Paulo: Elsevier Academic, 2012.',
    ],
    'ILM001': [
        '',
        '',
    ],
    'ISI002': [
        'ARLE, Ml e BERTOLA, D. Guia prático de Informática. Cronos, 2008.',
        'GARCIA, M. Informática aplicada a Negócios. SP: Brasport, 2005.',
        'MANZANO, A L N. G. Estudo Dirigido de Microsoft Office Excel 2007. Erica, 2007.',
        'SILVA, M.G. Informática: Terminologias Básicas. Erica, 2007.',
        'LAUDON, Kenneth C.; Laudon J.P. Sistemas de Informação. 5.ed. Rio de Janeiro: LTC, 2004.',
        'TURBAN, E; POTTER, R; RAINER JR, R K. Introdução a Sistemas de Informação.Campus, 2007',
    ],
    'LPO001': [
        'MARTINS, D S; ZILBERKNOP. Português Instrumental: de acordo com as atuais normas da ABNT. Atlas, 2009.',
        'CINTRA; CUNHA. Nova gramática do Português contemporâneo de acordo com a nova ortografia. Lexikon, 2009.',
        'FERREIRA, Aurélio Buarque de Holanda. Novo Dicionário Aurélio da Língua Portuguesa. Positivo, 2009.',
        'TEIXEIRA, Leonardo – Comunicação na Empresa – FGV Editora – 1ª Edição – 2006.',
        'MATOS, Gustavo Gomes – Comunicação Empresarial sem Complicação – Editora Manole – 1ª Edição – 2008.',
        'SERAFINI, Maria Teresa. Como Escrever Textos. São Paulo. Globo.1993.',
    ],
    'LIN100': [
        '',
        '',
    ],
};